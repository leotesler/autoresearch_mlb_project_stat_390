# Fit Boosted Tree Model with Best AutoResearch Iteration
...
# recipe ----
bt_rec <- recipe(success ~ ., mlb_train) |> 
...
# model specification ----
bt_spec <- boost_tree(mtry = tune(), min_n = tune(), learn_rate = tune(), trees = 500) |> 
  set_engine("xgboost") |> 
...
bt_grid <- grid_regular(extract_parameter_set_dials(bt_spec) |> 
...
total_models <- nrow(mlb_folds)*nrow(bt_grid)
...
# fit best model ----
optimal_wflow <- bt_tuned |> 
  extract_workflow() |> 
  finalize_workflow(select_best(bt_tuned, metric = "j_index"))

bt_model <- fit(optimal_wflow, mlb_train)

# save best model ----
dir.create("final_models/models")

save(bt_model, file = "final_models/models/bt_model.rda")