...
To find a set of features that best predict whether or not a minor league baseball player will reach the major leagues and achieve sustained success.

### Scope

The player population includes minor league hitters who have recorded at least 10 plate appearances since 2010. The data used comes from both Fangraphs.com's API and MLB's Stats API. 

A player is defined as a "successful" big league hitter if their average fWAR (Fangraphs Wins Above Replacement) per season is greater than or equal to 1, or if their fWAR per 162 games (a full season's worth of games) exceeds 1.5 and the player has played at least five big league seasons.

### Success Criterion

The AutoResearch loops improve cross-validated J Index by at least 0.05 over the baseline model that used standard features, and produces a stable and interpretable feature set.

As of this writing, improvement has stagnated around 0.025.

## Updated `program.md` (As of second loop)
...
results |> 
  ...
  datatable() |> 
  formatRound(columns = 3:4, digits = 6)
```

## Plan for next two weeks

This week, I'll go over my results and the agent's external data suggestions and see if it's worth running a third agent loop.

Next week, I'll write up my final report with my findings, and discuss whether or not my project was successful based on the success definition I've been working with.
